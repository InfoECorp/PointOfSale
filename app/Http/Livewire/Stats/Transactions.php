...
class Transactions extends Component
{
    ...
    public function mount()
    {
        ...
        $this->purchases_count = Purchase::where('date', '>=', now()->subWeek())
            ->select(DB::raw('CONVERT(date,GETDATE()) as date'), DB::raw('count(*) as purchases'))
            ->groupBy('date')
            ->pluck('purchases');

        $this->sales_count = Sale::whereDate('date', '>=', now()->subWeek())
            ->select(DB::raw('CONVERT(date,GETDATE()) as date'), DB::raw('count(*) as sales'))
            ->groupBy('date')
            ->pluck('sales');
    ...
    public function getDailyChartOptionsProperty()
    {
        ...
        $salesData = Sale::selectRaw('CONVERT(date,GETDATE()) as day, SUM(total_amount) as total_sales')
        ...
        $purchasesData = Purchase::selectRaw('CONVERT(date,GETDATE()) as day, SUM(total_amount) as total_purchases')
            ->whereBetween('date', [$currentMonth, Carbon::now()->endOfMonth()])
            ->groupBy('date')
            ->orderBy('date', 'ASC')
            ->get();
